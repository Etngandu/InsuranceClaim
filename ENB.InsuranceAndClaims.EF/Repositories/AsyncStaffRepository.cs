...
namespace ENB.InsuranceAndClaims.EF.Repositories
{
    ...
    public class AsyncStaffRepository: AsyncRepository<Staff>, IAsyncStaffRepository
    {
        ...
        public AsyncStaffRepository(InsuranceAndClaimsContext insuranceClaimsContext) : base(insuranceClaimsContext)
        {
            _insuranceClaimsContext = insuranceClaimsContext;
        }
        public IEnumerable<Staff> FindByName(string lastname)
        {
            return _insuranceClaimsContext.Set<Staff>().Where(x => x.LastName == lastname);
        }
    }
}
